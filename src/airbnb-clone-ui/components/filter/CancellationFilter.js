...
function CancelationFilter() {

    const [enabled, setEnabled] = useState(false)
    
    return (
    <Popover className="relative border-0">
        <Popover.Button className="focus:outline-none"><p className="filter-button">Free cancellation</p></Popover.Button>
        <Popover.Panel className="absolute z-10 w-screen max-w-[350px] px-4 mt-3 transform -translate-x-1/2 left-[110%] sm:px-0">
        <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
            <div className="bg-white">
              <div className="flex flex-grow justify-between items-center py-10 px-5">
                <div>
                    <p>Only show stays</p>
                    <p>that offer free cancellation</p>
                </div>
                <Switch
                    checked={enabled}
                    onChange={setEnabled}
...
export default CancelationFilter
